...
// import NavigationButton from "./NavigationButton";
import Link from "next/link";
import { ArrowDown, ArrowRightLeft, ChevronDown, ClipboardIcon, ClockIcon, Copy, DollarSignIcon, PlusSquareIcon } from "lucide-react";
import { QuickAction } from "@/types/commons";
import Toast from "../ui/Toast";
...
<div key={option.code} className="bg-white relative rounded-lg overflow-hidden shadow-sm min-h-[80px]">
  <div className={`w-12 h-[80%] md:w-14 absolute left-0 bottom-0 rounded-tr-2xl flex items-center justify-center text-white font-bold ${
  index === 1 ? 'bg-green-bg' : 'bg-cyan-dark'
}`}>
  <CountryFlag country={option.flag} />
</div>
  <div className="pl-16 md:pl-18 pr-3 sm:pr-4 py-3 flex-1 min-w-0">
    <p className="text-xs text-gray-500 mb-1">{option.name} Wallet</p>
    ...
    <div className="flex items-center gap-2">
      ...
      <button
      type="button"
        onClick={() => handleCopy(option.cardNumber, option.code)}
                ...
                <div className="relative flex-1" ref={fromDropdownRef}>
                  ...
                  >
                    <div className="flex items-center  gap-6">
                      <CountryFlag country={fromCurrency.flag} />
                       <span className="text-sm font-medium text-black capitalize">{fromCurrency.country}</span>
                      <div>
                         <span className="font-medium text-sm">{fromCurrency.code}</span>
                      <span className="text-sm font-medium text-black">{fromCurrency.balance}</span>
                      </div>
                      ...
                      {currencyOptions.map((option) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <CountryFlag country={option.flag} />
                            <div>
                              <span className="font-medium text-sm capitalize">{option.country}</span>
                ...
                <div className="relative flex-1" ref={toDropdownRef}>
                  ...
                  >
                    <div className="flex items-center gap-6">
                      <CountryFlag country={toCurrency.flag} />
                      {/* <span className="font-medium text-sm">{toCurrency.code}</span> */}
                      <span className="text-sm font-medium text-black capitalize">{toCurrency.country}</span>
                      <div>
                        <span className="font-medium text-sm">{toCurrency.code}</span>
                      <span className="text-sm font-medium text-black">{toCurrency.balance}</span>
                      </div>

...
export const NotificationsSection = () => {
  ...
  const Toggle: React.FC<ToggleProps> = ({ checked, onChange }) => (
    <label className="relative inline-flex items-center cursor-pointer">
      ...
      />
      <div
        className="w-12 h-7 rounded-full peer transition-colors duration-200 ease-in-out relative"
        style={{
      ...
      >
        <div
          className={`absolute top-0.5 left-0.5 w-6 h-6 rounded-full transition-transform duration-200 ease-in-out ${
            checked ? 'translate-x-5' : 'translate-x-0'
          }`}
  ...
  return (
    <div className="w-full px-6" style={{ backgroundColor: 'var(--jean-white)' }}>
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-semibold mb-2" style={{ color: 'var(--jean-gray-900)' }}>
          Notifications
        </h1>
        <p style={{ color: 'var(--jean-gray-600)' }}>
